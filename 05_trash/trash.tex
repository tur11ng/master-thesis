...
%        \item \textbf{Απόκρυψη (Hiding)} : $\forall \text{ distinct } m, m' \in M, r \sample \text{R}, \forall A \in \text{ μη-ομοιόμορφους PPT}:\\
%         \abs{Pr[A(\text{Commit}(m, r))=1] - Pr[A(\text{Open(r, m')})=1]} \leq ε_h$